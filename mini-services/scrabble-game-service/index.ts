...
interface MoveRecord {
  playerIndex: number;
  tiles: { row: number; col: number; tile: IPATile }[];
  score: number;
  words: string[];
}
...
const IPA_TILES: Omit<IPATile, 'id'>[] = [
...
function createTileBag(): IPATile[] {
  const bag: IPATile[] = [];
...
function drawTiles(tileBag: IPATile[], count: number): { tiles: IPATile[], remaining: IPATile[] } {
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', (data: { inviteCode: string; playerName: string }) => {
  ...
  socket.on('join-room', (data: { inviteCode: string; playerName: string }) => {
    const { inviteCode, playerName } = data;
  ...
  socket.on('pass-turn', (data: { inviteCode: string }) => {
